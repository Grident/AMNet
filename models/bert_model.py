...
class AMNetREModel(nn.Module):
    ...
    def get_matching_loss(self,images,aux_images,image_atts,image_feat,text_embeds,text_atts,text_feat):
        ...
        for b in range(bsz):
            ...
            image_embeds_neg.append(images.permute(1,0,2,3)[neg_idx]) #embed (bsz)*(13,50,768)
            image_atts_neg.append(image_atts[neg_idx]) #idx
        ...
        for b in range(bsz):
            neg_idx = torch.multinomial(weights_i2t[b], 1).item()#neg_idx
            text_embeds_neg.append(text_embeds[neg_idx]) ##text,(seq_len,embed) → (bsz)x(seq_len,embed)
            text_atts_neg.append(text_atts[neg_idx]) # (seq_len)→ (bsz)x(seq_len)
...
class AMNetNERModel(nn.Module):
    ...
    def get_matching_loss(self,images,aux_images,image_atts,image_feat,text_embeds,text_atts,text_feat):
        ...
        for b in range(bsz):
            neg_idx = torch.multinomial(weights_i2t[b], 1).item()#neg_idx
            text_embeds_neg.append(text_embeds[neg_idx]) ##(seq_len,embed) → (bsz)x(seq_len,embed)
            text_atts_neg.append(text_atts[neg_idx]) #mask (seq_len)→ (bsz)x(seq_len)
