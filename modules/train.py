        ...
        with torch.no_grad():
            with tqdm(total=len(self.dev_data), leave=False, dynamic_ncols=True) as pbar:
                ...
                for batch in self.dev_data:
                    ...
                    (loss,logits),labels,_ = self._step(batch, mode="dev")    # logits: batch, 3
        ...
        with torch.no_grad():
            with tqdm(total=len(self.test_data), leave=False, dynamic_ncols=True) as pbar:
                ...
                for batch in self.test_data:
                    batch = (tup.to(self.args.device)  if isinstance(tup, torch.Tensor) else tup for tup in batch)  # to cpu/cuda device  
                    (loss,logits),labels,i_path = self._step(batch, mode="dev")    # logits: batch, 3
                    total_loss += loss.detach().cpu().item()
                    
                    preds = logits.argmax(-1)
                    true_labels.extend(labels.view(-1).detach().cpu().tolist())
                    pred_labels.extend(preds.view(-1).detach().cpu().tolist())
                    img_path.extend(i for i in i_path)
                    pbar.update()    
                # evaluate done
                pbar.close()
                ...
                #         error_img = [img_path[i], str(list(self.re_dict.keys())[true_labels[i]]),str(list(self.re_dict.keys())[pred_labels[i]])]
                #         error_img = ', '.join(error_img)
                #         with open('MRE_Ours_no_ITC&ITM_error_output.txt','a') as f:
                #             f.write(error_img+'\n')
                #         f.close()

                # for i in range(len(pred_labels)):
                #     if pred_labels[i] == true_labels[i]:
                #         right_img = [img_path[i], str(list(self.re_dict.keys())[true_labels[i]]),str(list(self.re_dict.keys())[pred_labels[i]])]
                #         right_img = ', '.join(right_img)
                #         with open('MRE_Ours_no_ITC&ITM_right_output.txt','a') as f:
                #             f.write(right_img+'\n')
                #         f.close()   
...
class NERTrainer(BaseTrainer):
        ...
        with tqdm(total=self.train_num_steps, postfix='loss:{0:<6.5f}', leave=False, dynamic_ncols=True, initial=self.step) as pbar:
            ...
            for epoch in range(1, self.args.num_epochs+1):
                ...
                for batch in self.train_data:
                    self.step += 1
                    batch = (tup.to(self.args.device)  if isinstance(tup, torch.Tensor) else tup for tup in batch)
                    attention_mask, labels, logits, loss, _ = self._step(batch, mode="train")
                    avg_loss += loss.detach().cpu().item()
        ...
        with torch.no_grad():
            with tqdm(total=len(self.dev_data), leave=False, dynamic_ncols=True) as pbar:
                ...
                for batch in self.dev_data:
                    step += 1
                    batch = (tup.to(self.args.device)  if isinstance(tup, torch.Tensor) else tup for tup in batch)  # to cpu/cuda device
                    attention_mask, labels, logits, loss, _ = self._step(batch, mode="dev")    # logits: batch, seq, num_labels
                    total_loss += loss.detach().cpu().item()
    ...
    def test(self):
        ...
        with torch.no_grad():
            with tqdm(total=len(self.test_data), leave=False, dynamic_ncols=True) as pbar:
                ...
                for batch in self.test_data:
                    batch = (tup.to(self.args.device)  if isinstance(tup, torch.Tensor) else tup for tup in batch)  # to cpu/cuda device
                    attention_mask, labels, logits, loss, i_path = self._step(batch, mode="dev")    # logits: batch, seq, num_labels
                    total_loss += loss.detach().cpu().item()
                ...
                for i in range(len(y_true)):
                    if y_pred[i] != y_true[i]:
                        error_img = [img_path[i], str(y_true[i]), str(y_pred[i])]
                        error_img = ', '.join(error_img)
                        with open('twitter17_Ours_no_ITM_error_output.txt','a') as f:
                            f.write(error_img +'\n')
                        f.close() 
                ...
                for i in range(len(y_true)):
                    if y_pred[i] == y_true[i]:
                        error_img = [img_path[i], str(y_true[i]), str(y_pred[i])]
                        error_img = ', '.join(error_img)
                        with open('twitter17_Ours_no_ITM_right_output.txt','a') as f:
                            f.write(error_img +'\n')
                        f.close() 
    ...
    def _step(self, batch, mode="train"):
        ...
        return attention_mask, labels, logits, loss, i_path
    ...
    def multiModal_before_train(self):
        ...
        #main_task & matching_task lr
        main_task = re.compile(r'bert\.(encoder\.layer\.([6-9]|10|11)\..)')
        params = {'lr':self.args.lr, 'weight_decay':1e-2}
        params['params'] = []
        for name, param in self.model.named_parameters():
            if main_task.search(name) or 'encoder_conv' in name or 'alpha' in name \
                or 'itm_head' in name:
                params['params'].append(param)
        optimizer_grouped_parameters.append(params)
        ...
        for name, param in self.model.named_parameters(): # freeze ViT
            if 'image_model' in name:   
